module Lines
    ...
    function init_line_of_sight(psr)
        ...
        for phi in phis
            ...
            #if vec[2] <= theta_max
               p_end  = r_em * v̂
                p_surf = psr.r * v̂   # projection onto surface (good enough geometrically)
...
function init_line_of_sight!(psr; height=5e5, nphi=500)
    ...
    mag_axis ./= norm(mag_axis)

    r_em = psr.r + height

    theta_max = Functions.theta_max(psr.r_em / psr.r, psr)

    ζ = deg2rad(psr.alpha + psr.beta)

    psr.line_of_sight = Vector{Vector{Float64}}()

    phis = range(0, 2π, length=nphi)
    ...
    for φ in phis
        ...
        v = Transformations.rotate_back(rot_axis, v_rot)
        v̂ = v / norm(v)

        θ_mag = acos(clamp(dot(v̂, mag_axis), -1.0, 1.0))

        if θ_mag ≤ theta_max
            push!(psr.line_of_sight, r_em * v̂)
        end
    end
end
