...
module Geometry
    ...
    # ============================================================================
    # Based on:
    # - Blaskiewicz, Cordes & Wasserman (1991), ApJ, 370, 643
    # - Dyks, Rudak & Harding (2004), ApJ, 607, 939
    # ============================================================================

    """
        aberration_retardation_shift(r_em, P)

    Calculate the combined phase shift due to aberration and retardation effects.

    Following Blaskiewicz, Cordes & Wasserman (1991):
        Δφ_total = Δφ_ret + Δφ_ab ≈ 4π * r_em / (c * P)

    Both effects contribute approximately equally, each shifting by ~r_em/r_LC.
    ...
    # Returns
    - `Δφ::Float64`: total phase shift [radians]

    # Example
    ```julia
    # For r_em = 500 km, P = 1 s:
    Δφ = aberration_retardation_shift(500e3, 1.0)  # ≈ 0.02 rad ≈ 1.2°
    ```
    """
    function aberration_retardation_shift(r_em, P)
        return 4π * r_em / (c * P)
    end

    """
        retardation_phase_shift(r_em, P)

    Calculate phase shift due to light travel time from emission height.

    Photons emitted at height r_em arrive later than those from the surface,
    causing an apparent phase lag:
        Δφ_ret = 2π * r_em / (c * P) = r_em / r_LC
    ...
    # Returns
    - `Δφ::Float64`: retardation phase shift [radians]
    """
    function retardation_phase_shift(r_em, P)
        return 2π * r_em / (c * P)
    end

    """
        aberration_phase_shift(r_em, P)

    Calculate phase shift due to relativistic aberration from corotation.

    The magnetosphere corotates with the star. At emission height r_em,
    the corotation velocity causes aberration:
        Δφ_ab ≈ r_em / r_LC
    ...
    # Returns
    - `Δφ::Float64`: aberration phase shift [radians]
    """
    function aberration_phase_shift(r_em, P)
        return 2π * r_em / (c * P)
    end
    ...
    Aberration shifts the apparent emission direction due to corotation velocity.
    The shift is primarily in the azimuthal (ψ) direction and depends on the
    local corotation velocity v_rot = Ω × r.

    For a point at colatitude θ from the magnetic axis:
        β_rot = v_rot/c = (r_em * sin(θ)) / r_LC
    ...
    # Returns
    - `(θ_ab, ψ_ab)::Tuple{Float64, Float64}`: aberration-corrected angles [radians]
    ...
    """
        emission_points_with_ar(φ_array, α, β, r_em, P; 
                                include_aberration=true, 
                                include_retardation=true)

    Convert rotational phases to emission coordinates with aberration and 
    retardation corrections.

    This is the corrected version of `emission_points_from_phase` that accounts
    for relativistic effects in the rotating magnetosphere.

    # Physical Picture
    - **Retardation**: Observed phase φ_obs corresponds to emission that occurred
      at an earlier phase φ_em = φ_obs - Δφ_ret, because light takes time to 
      travel from emission height to observer.
    - **Aberration**: The emission direction is shifted due to the velocity of
      the corotating emission region.

    # Arguments
    - `φ_array::AbstractVector`: observed rotational phases [radians]
    - `α::Real`: magnetic inclination angle [radians]
    - `β::Real`: impact parameter [radians]
    - `r_em::Real`: emission height [m]
    - `P::Real`: rotation period [s]
    - `include_aberration::Bool`: apply aberration correction (default: true)
    - `include_retardation::Bool`: apply retardation correction (default: true)

    # Returns
    - `θ_array::Vector{Float64}`: colatitudes from magnetic axis [radians]
    - `ψ_array::Vector{Float64}`: azimuthal angles around magnetic axis [radians]
    - `φ_emission::Vector{Float64}`: emission phases (after retardation correction) [radians]
    ...
    φ_obs = generate_uniform_phase_array(100, α, β, r_em, P)
    θ, ψ, φ_em = emission_points_with_ar(φ_obs, α, β, r_em, P)
    
    # Compare with uncorrected:
    θ_no_ar, ψ_no_ar = emission_points_from_phase(φ_obs, α, β, r_em, P)
    ```
    """
    function emission_points_with_ar(φ_array, α, β, r_em, P; 
                                    include_aberration=true, 
                                    include_retardation=true)
        n = length(φ_array)
        θ_array = zeros(n)
        ...
        # Calculate A/R phase shifts
        if include_retardation
            Δφ_ret = retardation_phase_shift(r_em, P)
        else
            Δφ_ret = 0.0
        end
        
        if include_aberration
            Δφ_ab = aberration_phase_shift(r_em, P)
        else
            Δφ_ab = 0.0
        end
